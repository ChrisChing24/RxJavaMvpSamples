...
public abstract class CalendarView extends View {
    ...
    private Paint mRectPaint;//绘制矩形
    private int mCellSpace;//单元格横向间距
    private int mCellSpaceY;//单元格纵向间距
    public Row rows[] = new Row[TOTAL_ROW]; // 行数组，每个元素代表一行
    private int touchSlop;
    private boolean callBackCellSpace;
    public static CustomDate NowDate = new CustomDate();//自定义的日期，包括year,month,day
    ...
    public abstract void rightSlide(int position, int currIndex);

    public abstract void leftSlide(int position, int currIndex);
    ...
    private void init(Context context) {
        ...
        mCirclePaint.setColor(ContextCompat.getColor(getContext(), R.color.event_text_color));// 红色圆形
...
//            mTextPaint.setColor(ContextCompat.getColor(getContext(), R.color.cal_buckle_text_color));
            switch (recordState) {
                case Event:
                    mCirclePaint.setColor(ContextCompat.getColor(getContext(), R.color.event_text_color));
                    mTextPaint.setColor(ContextCompat.getColor(getContext(), R.color.white));
                    break;
                case Today:
                    mCirclePaint.setColor(ContextCompat.getColor(getContext(), R.color.title_bg));
                    mTextPaint.setColor(ContextCompat.getColor(getContext(), R.color.white));
                    break;
//                case UnSign:
//                    mCirclePaint.setColor(ContextCompat.getColor(getContext(), R.color.cal_safe_color));
//                    mRectPaint.setColor(ContextCompat.getColor(getContext(), R.color.red));
//                    break;
                case Unknown:
                    mCirclePaint.setColor(Color.TRANSPARENT);
                    mTextPaint.setColor(ContextCompat.getColor(getContext(), R.color.text_grayer));
                    break;
            }
...
//                    canvas.drawCircle(x, y, radius, mRectPaint);
                    //如果是重大事件或者今天，则用白色显示日期，其余日期颜色不变
                    if (recordState == RecordState.Today || recordState == RecordState.Event) {
                        mTextPaint.setColor(ContextCompat.getColor(getContext(), R.color.white));
                    } else {
                        mTextPaint.setColor(ContextCompat.getColor(getContext(), R.color.dark_calendar_text_color));
                    }
...
//                case CLICK_DAY:
//                    if (isNeedShowClickDay) {
//                        mTextPaint.setColor(ContextCompat.getColor(getContext(), R.color.text_near_black));
